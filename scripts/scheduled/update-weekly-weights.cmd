@echo off
rem Linggan scheduled task entry: update:weekly-weights (weekly events -> ranking weight snapshot)
...
set "WRAPPER_LOG=%~dp0..\..\data\run-logs\scheduled-update-weekly-weights-wrapper.log"
echo [%DATE% %TIME%] update-weekly-weights entry started >> "%WRAPPER_LOG%"
...
call npm run update:weekly-weights >> "%WRAPPER_LOG%" 2>&1
exit /b %ERRORLEVEL%
