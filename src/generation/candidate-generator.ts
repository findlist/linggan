...
export const scoreCandidate = (
  ...
  const total = metricNames.reduce(
    (sum, metric) => sum + metrics[metric] * config.weights[metric],
    0
  )
...
export const generateDailyCandidates = (
  ...
  const canGenerate =
    seeds.characters.length > 0 && seeds.scenes.length > 0 && seeds.elements.length > 0

  const candidates = canGenerate
    ? trends.slice(0, 10).flatMap((trend, trendIndex) =>
  ...
  return {
    ...
    summary: {
      ...
      ready_for_review: candidates.filter(
        candidate => candidate.score.total >= config.limits.publish_score
      ).length,
      auto_published: 0
    },
    candidates
