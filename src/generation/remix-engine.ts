...
const classifyBeat = (beat: string): BeatRole => {
  ...
  if (
    /建立|展示|开场|先行|平凡|微小偏差|线索|起跑|结果先行|闪回|叙述|结尾场景|同一选择点|自信开讲|外层故事开场|角色开始讲述|角色意识到|无声开场|以意象代替|起笔|歌唱/.test(
      beat,
    )
  )
    return 'opening'
  if (
    /铺垫|升级|重复|变化|采访|道具|目标|逐渐|积累|模仿|笨拙|倒推|原因链条|伏笔|暗线|切换|碎片|回溯|传递|结局A|结局B|分叉|矛盾|反应不符|内层故事|试图打破|暗流|唯一声音|意象叠加|细节延伸|延伸出人物|歌舞建立/.test(
      beat,
    )
  )
    return 'buildup'
  if (
    /冲突|受阻|矛盾|连锁|滚雪球|被迫|交汇|史诗|问题|打破|关键行为|错位|入侵|碰撞|关联|互补|出人意料|揭穿|内层结局|叙事结构|撕裂|凝结|裂痕|突发事件/.test(
      beat,
    )
  )
    return 'climax'
  if (
    /转折|破局|反转|偏离|揭露|真相|觉醒|意外|浮出|发现|互换理解|相反|暴露|揭示|拼合|还原|差异|闭环|自欺|彻底相反|反噬|博弈|沉默被|留白中转折|蔓延/.test(
      beat,
    )
  )
    return 'turning'
  if (
    /收尾|彩蛋|收场|回归|合并|散场|循环结束|认知已改变|重述|重构|重新定义|收束|首帧重现|观众被引导|选择最真实|外层因|重建新的|回归沉默|张力留存|余韵不绝|底色已变|终曲收束/.test(
      beat,
    )
  )
    return 'ending'
