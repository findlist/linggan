...
const renderTagList = (items, escapeHtml) =>
  items.length
...
const renderCharacterBody = (character, ctx) => {
  ...
  return [
      ...
      character.relationships.length
        ? character.relationships
            .map((id) => {
              ...
              return target
                ? renderEntityLink('characters', target.id, target.name, ctx.escapeHtml)
                : ctx.escapeHtml(id)
            })
            .join('')
    ...
    renderField(
      '别名',
      character.aliases.length ? ctx.escapeHtml(character.aliases.join('、')) : '<p class="detail-empty">无</p>',
    ),
...
const renderWorkBody = (work, ctx) => {
  ...
  return [
    ...
    renderField(
      '别名',
      work.aliases.length ? ctx.escapeHtml(work.aliases.join('、')) : '<p class="detail-empty">无</p>',
    ),
...
const renderMomentBody = (moment, ctx) => {
  ...
  return [
    ...
    renderField(
      '可复用节拍',
      `<ol class="detail-beats">${moment.reusable_beats.map((beat) => `<li>${ctx.escapeHtml(beat)}</li>`).join('')}</ol>`,
    ),
...
export const createDetailView = (ctx) => {
  ...
  root.addEventListener('click', (event) => {
    ...
    if (link) {
      open(link.dataset.detailLink, link.dataset.detailId)
      return
    }
