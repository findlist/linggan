...
const loadTrendExport = async () => {
  try {
    ...
    if (!response.ok) {
      try {
        const fallback = await fetch('/data/trend-export.json')
        if (fallback.ok) return await fallback.json()
      } catch {
        // fallback 路径失败时忽略，返回 null 让上层显示占位渠道
      }
      return null
    }
...
const pipelineNote = () =>
  `<div class="pipeline-note">${icon('shield', 18)}<p><b>不编造热度</b><br>无法核实的指标保持为空，具体 IP 只作参考标签。</p></div>`
