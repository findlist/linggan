 ...
 */
export const KnowledgeBatchSchema = z
  .object({
...
const moreConservativeRisk = (a: RiskLevel, b: RiskLevel): RiskLevel => (RISK_SEVERITY[a] >= RISK_SEVERITY[b] ? a : b)
...
const detectAliasConflicts = (knowledge: KnowledgeBase): string[] => {
  ...
  const checkCollection = (collection: string, items: Array<{ id: string }>, namesPerItem: string[][]) => {
  ...
  checkCollection(
    'works',
    knowledge.works,
    knowledge.works.map((w) => [w.title, ...w.aliases]),
  )
  checkCollection(
    'known_characters',
    knowledge.known_characters,
    knowledge.known_characters.map((c) => [c.name, ...c.aliases]),
  )
  checkCollection(
    'iconic_moments',
    knowledge.iconic_moments,
    knowledge.iconic_moments.map((m) => [m.name]),
  )
...
export const mergeKnowledgeBatches = async (input: {
  ...
  if (conflicts.length > 0) {
    throw new KnowledgeMergeError(`知识库合并失败：检测到 ${conflicts.length} 个别名冲突`, failures, conflicts)
  }

  // 原子写入：校验通过后先写临时文件再 rename，失败不破坏旧文件
  const store = new JsonDocumentStore<KnowledgeBase>(input.outputPath, KnowledgeBaseSchema, emptyKnowledgeBase)
