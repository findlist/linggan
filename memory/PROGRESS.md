...
**任务 B4：角色、作品和名场面详情（已完成）。**

**下一轮任务：B5 — 导出与收藏升级。**

选择理由：B4 已让用户能浏览素材详情并从详情页进入创作，但生成的混搭方案只能在页面内复制和简单收藏（localStorage 仅保存标题和钩子），无法导出为可分享、可归档的结构化文档，收藏列表也不支持查看详情、重新编辑或导出。按 `docs/DEVELOPMENT_DIRECTION.md` 第 10 轮"B5 导出与收藏升级"和验收条件"Markdown/JSON 导出，收藏可管理"，新增把混搭方案导出为 Markdown（人类可读）和 JSON（机器可读）两种格式的能力，并升级收藏管理让用户能查看已收藏方案的完整内容、删除和重新加载到工作台。本任务不依赖外部来源或新数据，可在现有生成引擎和收藏基础上独立验收。

验收条件：

- 混搭方案生成后可导出为 Markdown 文档，包含标题、概念、钩子、分镜列表、对白、画面提示词、发布文案和版权边界；
- 混搭方案可导出为 JSON 文档，结构化保存完整 RemixPlan 字段，便于后续重新加载或外部消费；
- 收藏列表升级：每条收藏保存完整方案（而非仅标题和钩子），支持点击查看详情、重新加载到工作台、单条导出和删除；
- 导出和收藏操作有明确的成功/失败反馈（toast 或状态提示）；
- 复用深色 Aurora 视觉，键盘可访问，移动端可用；
- 类型检查、全部测试和生产构建通过。
