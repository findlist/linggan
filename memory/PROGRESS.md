...
整体状态：本地数据闭环可验证；SQLite 已为默认存储，基础知识可幂等初始化、热点可事务入库；候选生成已接通 SQLite 正式趋势；正式趋势可原子导出为只读 JSON；网站热点雷达已消费真实趋势数据；候选已持久化到 SQLite，状态机支持 pending_review → approved/rejected → archived 流转和幂等键去重；今日推荐流已通过只读 JSON 导出消费 approved 候选；知识库增量合并命令已建立并通过真实批次验证，知识库已扩充至 9 部作品/19 角色/7 关系/11 名场面；尚无真实来源适配器和自动发布闭环
...
选择理由：B1 已将知识库扩充到 9 部作品、19 个角色和 11 个名场面，组合空间显著增加；但当前生成引擎的钩子只有 3 个模板、对白靠 hash 拼接，多生成几次即暴露套公式问题。按 `docs/DEVELOPMENT_DIRECTION.md` 5.1—5.4 节，升级钩子模板（4 类各 4+ 个，共 16+）、性格驱动对白（4 种性格类型）、按时长分镜（15/30/60s）和发布文案（3 标题+1 描述+3 标签），让生成结果差异化且可固定种子复现。

验收条件：

- 钩子模板至少 16 个，覆盖悬念/反差/提问/行动 4 类，按角色性格、场面节奏和目标时长选择；
- 性格驱动对白覆盖冷酷/热血/腹黑/温柔 4 种类型，从知识库角色的 `dialogue_style` 派生；
- 分镜按时长输出 15s（3 镜头）/30s（5 镜头）/60s（8 镜头），每镜头含时长、画面、动作、情绪；
- 每条方案输出 3 个标题候选、1 段 100 字描述和 3 个话题标签；
- 固定随机种子测试：同一输入与种子得到相同结果；20 条生成结果的规范化钩子唯一率不低于 70%；
- 类型检查、全部测试和生产构建通过。
