...
整体状态：本地数据闭环可验证；SQLite 已为默认存储，基础知识可幂等初始化、热点可事务入库；候选生成已接通 SQLite 正式趋势；正式趋势可原子导出为只读 JSON；网站热点雷达已消费真实趋势数据；候选已持久化到 SQLite，状态机支持 pending_review → approved/rejected → archived 流转和幂等键去重；今日推荐流已通过只读 JSON 导出消费 approved 候选；知识库增量合并命令已建立并通过真实批次验证，知识库已扩充至 9 部作品/19 角色/7 关系/11 名场面；跨作品混搭引擎已升级为多样化、固定种子可复现的生成器，支持 4 类共 24 个钩子模板、4 种性格驱动对白、按时长分镜（15/30/60s → 3/5/8 镜头）和发布文案（3 标题+描述+3 标签）；尚无真实来源适配器和自动发布闭环
...
**下一轮任务：B4 — 角色、作品和名场面详情页。**

选择理由：B2 已让生成引擎输出差异化、可制作化的钩子、对白、分镜和文案，但用户在素材库只能看到卡片，无法深入了解角色性格、作品类型和名场面结构，也无法从详情直接进入创作。按 `docs/DEVELOPMENT_DIRECTION.md` 3.2 节"没有详情页"问题与 B4 验收条件，新增可点击进入的详情视图（角色/作品/名场面三类），展示完整字段（角色类型、对白风格、关系、情绪弧、视觉动作等）并提供"开始创作"入口，把详情与现有混搭工作台打通，形成"浏览素材 → 理解结构 → 进入创作"的完整路径。本任务不依赖外部来源或新数据，可在现有知识库上独立验收。

验收条件：

- 素材库卡片可点击进入详情视图（弹窗或独立路由均可），覆盖角色、作品、名场面三种类型；
- 角色详情展示：所属作品、角色类型、特征、对白风格、关系、版权状态和来源证据；
- 作品详情展示：标题、别名、媒介类型、地区、上映年份、类型、版权状态和来源证据；
- 名场面详情展示：所属作品、冲突类型、情绪弧、视觉动作、可复用节拍、版权状态和来源证据；
- 每个详情页有"开始创作"入口，能把当前实体带入混搭工作台（如角色详情自动填入角色 A 或 B 选择器，名场面详情自动填入名场面选择器）；
- 详情视图复用深色 Aurora 视觉，键盘可访问，移动端可用；
- 类型检查、全部测试和生产构建通过。
