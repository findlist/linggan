...
整体状态：可构建；TypeScript 数据契约与基础自动测试已建立；尚未形成真实数据、API、数据库和自动发布闭环
...
| 测试体系 | 基础已建立 | 13 个数据契约测试通过；候选流水线和浏览器回归测试仍待补齐 |
...
1. 将候选生成器拆成可测试模块，CLI 只负责输入输出；
2. 设计本地持久化接口，为后续 PostgreSQL 迁移保留边界；
3. 建立发布状态机和幂等键；
4. 实现第一个基于固定样本的趋势来源适配器；
5. 为 `data/collection-inbox/` 增加正式 Schema、跨批次去重和迁移命令；
6. 将验证通过的暂存热点迁移到正式趋势存储，同时完整保留来源证据。
...
**任务：将候选生成器拆成可测试模块，并注入固定时钟。**

选择理由：数据契约已经建立，但当前候选生成仍集中在单个 CLI 脚本中并直接读取系统时间；拆分后才能以固定输入和固定时钟重复验证候选生成逻辑。

验收条件：

1. 评分和候选生成逻辑迁入独立 TypeScript 模块，CLI 仅负责读取配置、调用模块和输出结果；
2. 生成函数接收显式时钟，同一固定输入重复执行得到相同候选和时间字段；
3. 生成结果全部通过 `CandidateSchema`；
4. 自动测试至少覆盖正常输入、空趋势、重复执行和分数边界；
