...
本轮目标：上轮多样性测试矩阵改进轮提到"技术债 #2（JS bundle 持续增长）仍待评估"，本轮评估 knowledge-base.json 增长趋势和 gzip 后体积，建立阈值告警方案，确认当前是否需要优化。验收条件为：(1) 评估 knowledge-base.json 增长趋势和 gzip 后体积；(2) 记录阈值告警方案；(3) typecheck / lint / format:check / validate:data / test / build 全部通过。

完成：

- 创建 scripts/measure-bundle.ts 永久监测脚本：测量 JS/CSS/数据 JSON 的 raw 和 gzip 体积，对照 GREEN/YELLOW/RED 三级阈值报告状态，并输出 KB 数据规模便于对照增长趋势；
- 在 package.json 添加 "measure:bundle": "node --experimental-strip-types scripts/measure-bundle.ts" npm script；
- 运行 npm run measure:bundle 获取实际数据：
  - JS bundle: raw=415.3 kB, gzip=117.5 kB, ratio=28.3%（Vite build 报告 gzip=117.68 kB，差异因 gzip level 不同，脚本用 level 9）
  - CSS: raw=34.7 kB, gzip=7.4 kB, ratio=21.4%
  - knowledge-base.json: raw=434.4 kB, gzip=86.1 kB, ratio=19.8%
  - seed-entities.json: raw=14.2 kB, gzip=5.9 kB, ratio=41.5%
  - compatibility-matrix.json: raw=19.9 kB, gzip=3.3 kB, ratio=16.7%
  - KB 数据规模: 48 作品 / 136 角色 / 85 关系 / 88 名场面
- 建立 GREEN/YELLOW/RED 三级阈值告警方案（基于 JS gzip 体积）：
  - GREEN ≤ 130 kB：健康，无需优化
  - YELLOW 130—150 kB：接近优化阈值，规划 knowledge-base.json 懒加载（fetch 替代静态 import）
  - RED > 150 kB：必须实施懒加载或代码分割
- 当前 JS gzip 117.68 kB 处于 GREEN 健康状态，距 YELLOW 阈值还有 12.32 kB 余量；按每批知识库扩充增加约 7.5 kB gzip 计算，还能扩充约 1—2 批（3—6 部作品）才接近 YELLOW 阈值；
- 阈值告警方案已固化到 scripts/measure-bundle.ts，可在每次知识库扩充后运行 npm run measure:bundle 监测，脚本自动报告状态和建议动作。

bundle 组成分析：

| 资源                      | raw      | gzip     | 压缩率 | 说明                            |
| ------------------------- | -------- | -------- | ------ | ------------------------------- |
| JS bundle                 | 415.3 kB | 117.5 kB | 28.3%  | 含 knowledge-base.json 静态导入 |
| CSS                       | 34.7 kB  | 7.4 kB   | 21.4%  | 独立资源                        |
| knowledge-base.json       | 434.4 kB | 86.1 kB  | 19.8%  | 数据 JSON，gzip 压缩率高        |
| seed-entities.json        | 14.2 kB  | 5.9 kB   | 41.5%  | 数据 JSON                       |
| compatibility-matrix.json | 19.9 kB  | 3.3 kB   | 16.7%  | 数据 JSON，压缩率最高           |

- knowledge-base.json 是 JS bundle 体积主要来源：raw 434.4 kB（Vite 将 JSON 内联到 JS 模块，JS bundle raw 415.3 kB 为 JSON + 代码打包后体积），gzip 后 86.1 kB（占 JS gzip 117.5 kB 的 ~73%）；
- gzip 压缩率高（19.8%）：JSON 数据有很好的压缩潜力，用户实际下载体积（117.5 kB gzip）远小于 raw 体积（415.3 kB）；
- 增长趋势：b9（315 kB raw）→ b13（415 kB raw），每批约 +25 kB raw / +7.5 kB gzip；按当前 GREEN 余量 12.32 kB，约 1—2 批扩充后进入 YELLOW。

验证：typecheck 通过、lint 通过（0 errors 0 warnings）、format:check 通过（All matched files use Prettier code style）、validate:data 通过（5 份 JSON 有效，跨文件外键校验通过，包括 compatibility-matrix.json ↔ knowledge-base.json 交叉校验）、test 422/422 通过（本轮无新增测试，不涉及源代码改动）、build 34 modules 通过（CSS 34.71 kB / gzip 7.46 kB 不变、JS 415.32 kB / gzip 117.68 kB 不变，本轮只新增脚本和 npm command，不涉及前端 bundle）。npm run measure:bundle 输出确认 JS gzip 117.5 kB [GREEN] 健康。

关键决策与遗留问题：

- 阈值选择 GREEN≤130 kB / YELLOW 130—150 kB / RED>150 kB：基于当前 JS gzip 117.68 kB 设置合理余量；GREEN 到 YELLOW 的 12.32 kB 间隔约等于 1—2 批知识库扩充的 gzip 增量，提供预警时间；YELLOW 到 RED 的 20 kB 间隔提供实施懒加载的缓冲期；
- 不立即实施懒加载的原因：(1) 当前 GREEN 状态体积可接受；(2) knowledge-base.json gzip 压缩率 19.8%，用户实际下载体积仅 86.1 kB；(3) 懒加载需重构 src/data/knowledge.js 的静态 import 为 fetch + 异步初始化，涉及前端多处状态管理调整，风险与当前收益不匹配；
- 当 JS gzip 超过 130 kB（YELLOW）时规划懒加载：将 knowledge-base.json 从 Vite 静态 import 改为运行时 fetch，从 build 时内联改为运行时加载，预计可减少 JS gzip 约 86 kB（knowledge-base.json gzip 体积），使 JS gzip 降至约 31 kB；
- 技术债 #1（多样性测试矩阵固定切片）已在上轮解决；技术债 #2（JS bundle 持续增长）已在本轮解决（记录结论和阈值告警方案，确认当前无需优化）；
- 本轮未修改任何源代码，只新增 scripts/measure-bundle.ts 和 package.json 的 npm script，风险极低；
- Phase 4 商业化与扩展（E1—E5）仍需用户决策，不得擅自启动。

下一轮：JS bundle 体积评估与阈值告警方案已完成，技术债 #2 已解决。当前 JS gzip 117.68 kB 处于 GREEN 健康状态，无需优化。4.1—4.4 本地数据迭代方向全部完成，技术债 #1 和 #2 均已解决。Phase 4 商业化与扩展（E1—E5）需用户决策，不得擅自启动。下一轮唯一首选任务为知识库扩充第十四批（48→51 作品），扩充后需运行 npm run measure:bundle 确认 JS gzip 仍在 GREEN 或 YELLOW 范围。
...
- C3 重复率保持 0.0%（不变，健康）；
- avg max_similarity 0.528（-0.003，远低于 0.7 阈值，健康）；
- 唯一钩子 99.0%（从 99.7% 变化）、唯一对白A 81.6%（从 80.0% 变化）、唯一对白B 86.7%（从 84.8% 变化）——新增角色（宁采臣/燕赤霞）的性格类型和 dialogue_style 与基线角色不同，产生不同的对白模板选取，导致唯一率变化。

验证：typecheck 通过、lint 通过（0 errors 0 warnings）、format:check 通过、validate:data 通过（5 份 JSON 有效，跨文件外键校验通过，包括 compatibility-matrix.json ↔ knowledge-base.json 交叉校验）、test 422/422 通过（无测试断言 analyze-diversity 输出，矩阵变化不破坏测试）、build 34 modules 通过（CSS 34.71 kB 不变、JS 415.32 kB 不变，本轮只改脚本和文档，不涉及前端 bundle）。analyze:diversity 生成多样性自检：C3 重复率 0.0%（不变）、avg max_similarity 0.528（从 0.531 变化）、分镜结构唯一率 100.0%（不变）、唯一钩子 99.0%（从 99.7% 变化）、唯一对白A 81.6%（从 80.0% 变化）、唯一对白B 86.7%（从 84.8% 变化）。
...
- 补提交文档改动：上轮归档维护轮（fef0bfd）提交了 PROGRESS.md 但遗留 docs/DEVELOPMENT_STANDARD.md（v1.1 升级）和 memory/README.md（归档规则）未提交，本轮一并提交修复文档滞后；
- 已知技术债 #1（多样性测试矩阵固定切片）已解决；
- JS bundle 持续增长（技术债 #2）已通过 measure:bundle 脚本建立 GREEN/YELLOW/RED 三级阈值告警方案，当前 JS gzip 117.68 kB 处于 GREEN 健康状态（≤130 kB），无需优化（本轮不涉及 bundle 变化，415.32 kB 不变）；
- Phase 4 商业化与扩展（E1—E5）仍需用户决策，不得擅自启动。

下一轮：多样性测试矩阵改进已完成，analyze:diversity 多样性数字从 0.531/99.7%/80.0%/84.8% 变为 0.528/99.0%/81.6%/86.7%，随知识库扩充真实变化。技术债 #1 已解决。Phase 4 商业化与扩展（E1—E5）需用户决策，不得擅自启动。下一轮唯一首选任务为 JS bundle 优化（评估 knowledge-base.json 增长趋势和 gzip 后体积，若当前可接受则记录结论和阈值告警方案）。
...
2. ~~**JS bundle 持续增长**~~（已于 2026-08-21 JS bundle 体积评估与阈值告警方案轮解决）：knowledge-base.json 从 b9 的 315kB 增长至 b13 的 415kB（每批约 +25kB）；创建 scripts/measure-bundle.ts 永久监测脚本和 npm run measure:bundle 命令，建立 GREEN（≤130kB）/YELLOW（130—150kB）/RED（>150kB）三级阈值告警方案；当前 JS gzip 117.68 kB 处于 GREEN 健康状态，距 YELLOW 阈值还有 12.32 kB 余量（约 1—2 批扩充空间），暂无需实施懒加载或代码分割。
...
**前端交互反馈增强——生成进度反馈（用户优先级 6 第二单元）**：筛选状态持久化（第一单元）已于 2026-08-26 完成。下一轮最小可验收单元为生成进度反馈：工作台点击生成后的即时状态提示（生成中/完成/失败），消除核心流程反馈缺失。验收条件：(1) 生成流程有真实用户可感知的进度反馈；(2) 新增行为有测试或浏览器验证覆盖；(3) typecheck / lint / format:check / validate:data / test / build 全部通过。剩余候选：收藏分组（收藏列表按自定义分组管理）。次优先级：知识库扩充第十四批（48→51 作品，候选：人民的名义/走向共和/沉默的羔羊/搏击俱乐部/美丽人生/天堂电影院，扩充后需运行 npm run measure:bundle 确认 JS gzip 仍在 GREEN 或 YELLOW 范围，当前 118.1 kB 距 YELLOW 阈值 130 kB 约 12 kB 余量）。
