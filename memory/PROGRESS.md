...
最后更新：2026-07-29
当前轮次：M1 基础知识库与热点入库闭环轮
当前阶段：Phase 1 — 本地内容数据基础验证
整体状态：本地数据闭环可验证；已具备基础知识库、采集批次校验、跨批次去重和正式趋势 JSON 入库，尚无生产数据库、真实来源适配器、网站消费和自动发布闭环
...
| 内容图谱 | 基础库可校验 | 已有作品、6 个知名人物、3 组关系和3个抽象名场面样本；具体知名内容均为 `reference_only` |
| 热点采集 | 本地入库闭环完成，任务恢复启用 | 每天 07:30、13:30、19:30 采集；批次经 Schema、跨批次去重和迁移进入正式趋势 JSON，真实批次尚未产生 |
| 本地持久化 | 通过 | `TrendStore` 与 JSON 实现解耦，Schema 校验、原子替换、幂等与坏批次隔离测试通过 |
| 自动发布 | 未实现 | 配置存在，但没有发布状态机和存储 |
| 行为分析 | 未实现 | 尚未采集曝光、复制和成片事件 |
| 测试体系 | 基础验证通过 | 24 个测试覆盖数据契约、生成、非法批次、重复、部分失败、幂等和写入保护；浏览器回归仍待补齐 |
...
1. 让每日候选流水线读取正式趋势存储，并在空库时明确回退或停止；
...
**任务：让每日候选流水线读取 `data/stores/trends.json` 中的正式趋势记录。**
