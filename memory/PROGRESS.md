...
整体状态：本地数据闭环可验证；SQLite 已为默认存储，基础知识可幂等初始化、热点可事务入库；候选生成已接通 SQLite 正式趋势；正式趋势可原子导出为只读 JSON 供前端消费；尚无真实来源适配器和自动发布闭环
...
**任务 A2：正式趋势查询与前端导出边界（已完成）。**

**下一轮任务：B6 — 让网站热点雷达消费正式趋势导出 JSON。**

选择理由：A2 已建立 SQLite 到静态网站的安全导出边界，下一步让前端热点雷达读取导出文件，替换“等待真实采集批次”占位提示，用户可看到真实、可追溯的热点数据。
