...
1. **多样性测试矩阵固定切片**：scripts/analyze-diversity.ts 使用 known_characters.slice(0,5) + 10 个 original 角色 = 15 角色和 iconic_moments.slice(0,3) = 3 名场面固定矩阵，导致 b9—b13 五轮多样性数字完全不变（0.531/100.0%），无法反映新增 9 角色 + 6 名场面的真实价值。需改进测试矩阵让新增数据进入统计。
...
**多样性测试矩阵改进**：改进 scripts/analyze-diversity.ts 让新增知识库数据进入统计，使多样性指标随知识库扩充真实变化。验收条件为：(1) 测试矩阵覆盖最近一批新增角色和名场面；(2) b9—b13 的多样性数字不再完全不变；(3) typecheck / test / build 全部通过。
