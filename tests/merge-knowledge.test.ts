...
import {
  ...
  KnowledgeMergeError,
  mergeKnowledgeBatches
} from '../src/knowledge/merge-knowledge.ts'
...
test('merges works and characters from separate batches into valid knowledge base', async () => {
  await withTemporaryDirectory(async directory => {
    ...
    await mkdir(inbox)
    await writeFile(join(inbox, '01-works.json'), JSON.stringify({
      schema_version: 1,
      works: [createWork()]
    }))
    await writeFile(join(inbox, '02-characters.json'), JSON.stringify({
      schema_version: 1,
      known_characters: [createCharacter(), createCharacter({
...
test('cross-batch same id merges sources aliases and traits', async () => {
  await withTemporaryDirectory(async directory => {
    ...
    await writeFile(join(inbox, '00-work.json'), JSON.stringify({
      schema_version: 1,
      works: [createWork()]
    }))
    await writeFile(join(inbox, '01-char.json'), JSON.stringify({
      schema_version: 1,
      known_characters: [createCharacter({
        aliases: ['角色甲', '初始别名'],
        traits: ['勇敢'],
    ...
    await writeFile(join(inbox, '02-char.json'), JSON.stringify({
      schema_version: 1,
      known_characters: [createCharacter({
        aliases: ['角色甲', '补充别名'],
        traits: ['谨慎', '勇敢'],
...
test('risk level takes the more conservative value on merge', async () => {
  await withTemporaryDirectory(async directory => {
    ...
    const outputPath = join(directory, 'knowledge-base.json')
    await mkdir(inbox)
    await writeFile(join(inbox, '01-work.json'), JSON.stringify({
      schema_version: 1,
      works: [createWork({ risk_level: 'low' })]
    }))
    await writeFile(join(inbox, '02-work.json'), JSON.stringify({
      schema_version: 1,
...
test('migration skips a bad batch while importing valid batches', async () => {
  await withTemporaryDirectory(async directory => {
    ...
    const outputPath = join(directory, 'knowledge-base.json')
    await mkdir(inbox)
    await writeFile(join(inbox, 'valid.json'), JSON.stringify({
      schema_version: 1,
      works: [createWork()]
    }))
...
test('batch schema rejects duplicate aliases within one entity', async () => {
  await withTemporaryDirectory(async directory => {
    ...
    await writeFile(join(inbox, 'batch.json'), JSON.stringify({
      schema_version: 1,
...
test('alias conflict between different entities fails merge without writing', async () => {
  await withTemporaryDirectory(async directory => {
    ...
    const outputPath = join(directory, 'knowledge-base.json')
    await mkdir(inbox)
    await writeFile(join(inbox, 'conflict.json'), JSON.stringify({
      schema_version: 1,
      works: [
        createWork({ id: 'work_a', title: '作品甲', aliases: ['冲突别名'] }),
        createWork({ id: 'work_b', title: '作品乙', aliases: [], sources: [{
    ...
    await assert.rejects(
      mergeKnowledgeBatches({ inboxDirectory: inbox, outputPath }),
      KnowledgeMergeError
    )
...
test('merged document passes full knowledge base schema with relationships and moments', async () => {
  await withTemporaryDirectory(async directory => {
    ...
    const outputPath = join(directory, 'knowledge-base.json')
    await mkdir(inbox)
    await writeFile(join(inbox, 'full.json'), JSON.stringify({
      schema_version: 1,
      works: [createWork()],
      known_characters: [
        createCharacter(),
...
test('invalid merge preserves the previous knowledge base file', async () => {
  await withTemporaryDirectory(async directory => {
    ...
    await writeFile(join(inbox, 'bad.json'), JSON.stringify({
      schema_version: 1,
...
test('base document entities merge with inbox by id without duplication', async () => {
  await withTemporaryDirectory(async directory => {
    ...
    await writeFile(join(inbox, 'update.json'), JSON.stringify({
      schema_version: 1,
