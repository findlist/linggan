...
- 先以 `data/knowledge-base.json` 建立可校验的作品、知名人物、人物关系和抽象名场面基础库；具体知名内容统一为 `reference_only`。
...
- 每批采集先通过 `CollectionBatchSchema`，再按稳定指纹跨批次去重并通过事务迁移到 SQLite；坏批次隔离报告，不能阻止其他有效批次入库。
- 聚合相同事件，记录多来源证据和热度变化。
- 识别人物、作品、梗、动作、道具和情绪。
...
- 作品、知名人物、关系和抽象名场面具有运行时 Schema 与首批可验证 JSON；
- 采集批次具有严格 Schema，非法来源、重复 ID 和计数不一致会失败；
- 正式趋势存储通过接口与 SQLite 隔离，可由 PostgreSQL 适配器替换；
- `collection-inbox` 可迁移到正式趋势存储，支持跨批次去重、幂等、部分失败和原子写入；
- 采集任务每轮写入批次后自动执行迁移，只有校验通过的数据进入正式趋势库。

该里程碑只代表本地数据闭环可验证，不代表生产数据库、真实来源覆盖、网站消费和自动发布已经完成。

### 阶段 C：创意引擎（第 5—6 周）

- 已完成基础跨作品灵感工作台：网站直接消费知识库 JSON，支持角色 A × 角色 B × 抽象名场面 × 风格 × 时长组合、随机生成、原创改写对白、复制和本地收藏。
...
1. 每日候选流水线读取 SQLite 正式趋势，空库时明确停止或返回空结果，不伪造热点；
2. 接入首个公开且允许访问的真实来源适配器，保留可复核来源证据；
3. 为网站提供正式趋势查询边界，让热点雷达和组合器消费已校验数据；
4. 增加桌面与移动端浏览器回归，覆盖趋势筛选、组合生成、随机生成、复制和收藏。

集成完成条件：公开来源产生的批次能够依次经过 Schema、跨批次去重和 SQLite 事务入库，被候选生成与网站读取；任一环节为空、失败或来源不可核实时均有显式状态，不回退到伪造数据。
